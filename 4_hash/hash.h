    ...
    public:
        ...
        */
        CHash(int hashTableSize, int default_block_size)
            : m_tableSize(hashTableSize), 
              m_Memory(std::move(CMemoryManager<leaf>(default_block_size, true)))
        {   
        ...
        */
        bool update(T* pElement)
        {
            ...
            unsigned int idx = 0;
            leaf* old_node = findLeaf(pElement, idx);
            if (nullptr != old_node)
            {
                old_node->pData = pElement;
                return true;
            }
            leaf* new_node = nullptr;
            try
            {
                new_node = m_Memory.newObject();
            }
            catch (...)
            {
                throw CMemoryException();
            ...
            new_node->pnext = m_pTable[idx];
            m_pTable[idx] = new_node;
            return false;
        }
        ...
        {
            unsigned int idx = 0;
            leaf* node = findLeaf(&pElement, idx);
            return (nullptr == node) ? nullptr : node->pData;
        }
        ...
        */
        ...
        {
            unsigned int idx = 0;
            leaf* node = findLeaf(&element, idx);
            if (nullptr == node)
            {
                return false;
            }
            ...
            bool removed = m_Memory.deleteObject(node);
            _ASSERT(true == removed);
            return true;
        }
